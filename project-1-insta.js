...
handleEvent(e){
  ...
  async loadFoxIntoSlide(slide) { //Takes this.slide as input
    try {
      const response = await fetch("https://randomfox.ca/floof/"); //Does the fetching of fox images.  /api/response.json"
      ...
      img.src = data.image; // [this.currentIndex].src Uses currentIndex to change which image is wanted based on its position in the images array in response.json. Connects to the "src" element in each array entry.
      img.alt = "Random Fox";
